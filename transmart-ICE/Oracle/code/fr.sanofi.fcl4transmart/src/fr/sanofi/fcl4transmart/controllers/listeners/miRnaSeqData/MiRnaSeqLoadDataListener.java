...
import fr.sanofi.fcl4transmart.model.classes.workUI.miRnaSeqData.MiRnaSeqLoadDataUI;
import fr.sanofi.fcl4transmart.model.interfaces.DataTypeItf;

public class MiRnaSeqLoadDataListener extends LoadDataListener implements Listener {

	public MiRnaSeqLoadDataListener(MiRnaSeqLoadDataUI loadDataUI, DataTypeItf dataType){
	...
	protected void setJobMetadata(Job job) throws Exception{
		...
		job.getJobMeta().setParameterValue("MAP_FILENAME", ((MiRnaSeqData)dataType).getMappingFile().getName());
	...
	protected String createUnixCommand() throws Exception{
		...
		command+="-param:MAP_FILENAME="+((MiRnaSeqData)dataType).getMappingFile().getName()+" ";
		command+="-param:DATA_TYPE="+"R ";
		command+="-param:MIRNA_TYPE="+"MIRNA_SEQ ";
		String dimensionMap="";
		if(((HDDData)dataType).getDimFile()!=null) dimensionMap=((HDDData)dataType).getDimFile().getName();
		else dimensionMap=((HDDData)dataType).getMappingFile().getName();
