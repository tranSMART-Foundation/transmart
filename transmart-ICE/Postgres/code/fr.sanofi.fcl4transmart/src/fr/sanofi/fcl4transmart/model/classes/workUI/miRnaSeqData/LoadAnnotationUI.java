...
import fr.sanofi.fcl4transmart.controllers.listeners.miRnaSeqData.CheckAnnotationListener;
import fr.sanofi.fcl4transmart.controllers.listeners.miRnaSeqData.MiRnaSeqLoadAnnotationListener;
...
public class LoadAnnotationUI implements WorkItf, LoadAnnotationUIItf{
	...
	public void addLoadPart(boolean editable){
		...
		if(RetrieveData.testTm_czConnection() && RetrieveData.testTm_lzConnection() && RetrieveData.testDeappConnection()){
			loadButton.addListener(SWT.Selection, new MiRnaSeqLoadAnnotationListener(this, this.dataType));
		}
		else{
	...
	public void setAnnotationLogFile(File log) {
		((MiRnaSeqData)this.dataType).setAnnotationFile(log);
	}
}
