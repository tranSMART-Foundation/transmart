				...
				try{
					...
					try{
					  ...
					  try{
							...
							Connection metacon = DriverManager.getConnection(connection, PreferencesHandler.getDemodataUser(), PreferencesHandler.getDemodataPwd());
							...
							if(rs.next()){
								int n=rs.getInt(1);
								rs=metastmt.executeQuery("insert into tm_concept_counts (concept_path, parent_concept_path, patient_count) values('"+topNode+"', '"+programNode+"', "+n+")");
							}
							...
							for(int i=3; i<splited.length-1; i++){
								if(splited[i].compareTo("")!=0){
									...
									if(rs.next()){
										int n=rs.getInt(1);
										rs=metastmt.executeQuery("insert into tm_concept_counts (concept_path, parent_concept_path, patient_count) values('"+node+"', '"+parentNode+"', "+n+")");
										parentNode=node;
									}				
								}
							}
