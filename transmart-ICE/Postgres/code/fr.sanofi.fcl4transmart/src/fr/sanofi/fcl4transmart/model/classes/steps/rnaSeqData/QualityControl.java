...
import fr.sanofi.fcl4transmart.model.classes.workUI.rnaSeqData.QualityControlUI;
import fr.sanofi.fcl4transmart.model.interfaces.DataTypeItf;
import fr.sanofi.fcl4transmart.model.interfaces.StepItf;
import fr.sanofi.fcl4transmart.model.interfaces.WorkItf;
/**
 *This class represents the quality control step for RNAseq data
 */	
public class QualityControl implements StepItf{
	...
	public String getDescription(){
		return "This step allows controlling RNAseq data loading quality.\n"+
				"The intensity values for transcript are displayed for all samples, coming from the raw data file and from the database.\n"+
				"A transcript has to be chosen. This transcript name has to be present in the raw RNAseq data file.\n"+
	...
	public boolean isAvailable(){
		try{
			if(((RnaSeqData)this.dataType).getRawFiles()==null || ((RnaSeqData)this.dataType).getRawFiles().size()==0){
				return false;
			}
			File stsmf=((RnaSeqData)this.dataType).getMappingFile();
			if(stsmf==null){
				return false;
			}
			...
			if(((RnaSeqData)this.dataType).getLogFile()==null){
				return false;
			}
			return true;
		}
