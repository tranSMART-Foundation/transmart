...
import fr.sanofi.fcl4transmart.controllers.listeners.rnaSeqData.RnaSeqQCListener;
...
/**
 *This class allows the creation of the composite for RNAseq data quality control
 */
public class QualityControlUI implements WorkItf{
	...
	private RnaSeqQCListener controller;
	...
	public Composite createUI(Composite parent){
		...
		new Thread(){
			public void run() {
				number=String.valueOf(RetrieveData.getRnaSeqTranscriptNumber(dataType.getStudy().toString()));
		...
		complete.addListener(SWT.Selection, new Listener(){
			...
			public void handleEvent(Event event) {
				if(testDeapp){
					...
					new Thread(){
						public void run() {
							completeWorked=(new RnaSeqQCListener(dataType).writeLog());
	...
	public Composite createBody(String transcript){
		...
		for(File rawFile: ((RnaSeqData)this.dataType).getRawFiles()){
		...
		controller=new RnaSeqQCListener(this.dataType);
		Shell shell=new Shell();
		shell.setSize(50, 100);
		GridLayout gridLayout=new GridLayout();
		gridLayout.numColumns=1;
