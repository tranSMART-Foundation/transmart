...
public class etlPreferences_back {
	private Text host;
	private Text port;
	private Text user;
	private Text pass;
	...
	public etlPreferences_back(){
		...
		try {
	        ...
	        node = securePref.node("etl");
	               
            staticHost=node.get("host", "");
            staticPort=node.get("port", "");
        	staticUser=node.get("user", "");
        	staticPass=node.get("password", "");
        	staticKettleDirectory=node.get("kettleDirectory", "");
        	staticJobsDirectory=node.get("jobsDirectory", "");
        	staticFilesDirectory=node.get("filesDirectory", "");
	...
	public void execute(Display display) {		
		...
		ok.addListener(SWT.Selection, new Listener(){
			...
			public void handleEvent(Event event) {
            	...
            	try{
		            node.put("kettleDirectory", kettleDirectory.getText(), true);
            	 } catch (StorageException e1) {
			            e1.printStackTrace();
			     }
            	...
            	try{
		            node.put("jobsDirectory", jobsDirectory.getText(), true);
            	 } catch (StorageException e1) {
			            e1.printStackTrace();
			     }
            	...
            	try{
		            node.put("filesDirectory", filesDirectory.getText(), true);
            	 } catch (StorageException e1) {
			            e1.printStackTrace();
			     }
