...
public class ConfigXmlUnmarshaller {
     ...
     */
    private static final String ELEMENT_SIMPLE_DAS_SERVER = "simpledasserver";
    ...
    /**
     * String defining the namespace for simpledasserverconfig.xml.
     */
    ...
    /**
     * This method parses the /simpledasserver element.
    ...
    /**
     * This method parses the /simpledasserver/global element .
     * @param xpp the current XmlPullParser instance
    ...
    private GlobalConfiguration processGlobalElement(XmlPullParser xpp) throws IOException, XmlPullParserException, ConfigurationException {
            ...
            if (xpp.getEventType() == XmlPullParser.START_TAG) {
				...
				if (ELEMENT_BASEURL.equals(tagName)) {
                    ...
                    if (url == null || url.length() == 0){
                        throw new ConfigurationException ("Please check your XML configuration file.  No value has been given for the /simpledasserver/global/baseurl element.");
                    }
                    ...
                    if (! baseURL.endsWith("/das/")){
                        logger.error ("The value (URL) given for /simpledasserver/global/baseurl in the SimpleDasServerConfig.xml file (given as '" + url + "') should end with /das/");
                        throw new ConfigurationException("The value (URL) given for /simpledasserver/global/baseurl in the SimpleDasServerConfig.xml file (given as '" + url + "') should end with /das/");
                    }
                }
        ...
        if (defaultStylesheet == null || defaultStylesheet.length() == 0){
            throw new ConfigurationException ("Please check your XML configuration file.  No value has been given for the /simpledasserver/global/default-stylesheet element.");
        }
    ...
    /**
     * This method parses the /simpledasserver/datasources element .
     * @param xpp the current XmlPullParser instance
    ...
    private Map<String, DataSourceConfiguration> processDataSources(XmlPullParser xpp)
        ...
        if (dataSourceConfigList.size() == 0){
            throw new ConfigurationException("Please check your XML configuration file.  At least one data source must be configure under the /simpledasserver/datasources element.");
        }
    ...
    /**
     * This method parses a single /simpledasserver/datasources/datasource element .
     * @param xpp the current XmlPullParser instance
     * @return a List<DataSourceConfiguration> collection that contains all of the data source specific configuration.
     * @throws XmlPullParserException in the event of a fatal error in the XML file, e.g. not being well-formed.
     * @throws IOException in the event of a problem with reading the XML file.
    ...
    private DataSourceConfiguration processDataSource(XmlPullParser xpp)
        ...
        try{
            ...
            throw new ConfigurationException ("Please check the XML configuration file.  The URL '" + hrefString + "' that has have given in the /simpledasserver/datasources/datasource/@description-href attribute is not valid.", murle);
        }
        ...
        if (className == null || className.length() == 0){
            throw new ConfigurationException("Please check your XML configuration file.  No value has been given for one of the /simpledasserver/datasources/datasource/class elements.");
        }

        if (id == null || id.length() == 0){
            throw new ConfigurationException("Please check your XML configuration file.  No value has been given for one of the mandatory /simpledasserver/datasources/datasource/@id attributes.");
        }

        if (name == null || name.length() == 0){
            throw new ConfigurationException("Please check your XML configuration file.  No value has been given for one of the mandatory /simpledasserver/datasources/datasource/@name attributes.");
        }

        if (mapmaster == null || mapmaster.length() == 0){
            throw new ConfigurationException("Please check your XML configuration file.  No value has been given for one of the mandatory /simpledasserver/datasources/datasource/@mapmaster attributes.");
        }
