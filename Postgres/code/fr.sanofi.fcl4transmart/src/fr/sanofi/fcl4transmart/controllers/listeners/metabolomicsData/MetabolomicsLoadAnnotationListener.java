...
import fr.sanofi.fcl4transmart.model.classes.workUI.metabolomicsData.MetabolomicsLoadAnnotationUI;
import fr.sanofi.fcl4transmart.model.interfaces.DataTypeItf;

/**
 *This class controls the platform annotation loading 
 */	
public class MetabolomicsLoadAnnotationListener extends LoadAnnotationListener implements Listener{
	public MetabolomicsLoadAnnotationListener(MetabolomicsLoadAnnotationUI ui, DataTypeItf dataType){
		super(ui, dataType);
	}
	@Override
	protected void setParameters(){
		newPath=dataType.getPath().getAbsolutePath()+File.separator+platformId+"_annotation.txt";
		storedProcedureLaunched=".*Starting entry \\[run_i2b2_load_metabolomics_annot\\].*";;
		storedProcedreEnded=".*Finished job entry \\[run_i2b2_load_metabolomics_annot\\].*";
		queryStoredProcedureStarted="select max(JOB_ID) from CZ_JOB_AUDIT where STEP_DESC='Starting I2B2_LOAD_METABOLOMICS_ANNOTTATION'";
		queryStoredProcedureEnded="select * from cz_job_audit where (step_desc like '%End I2B2_LOAD_METABOLOMICS_ANNOT%' or step_status='FAIL') and job_id=";
		unixDir="metabolomics";
		checkPs=".*load_metabolomics_annotation.*";
	}
	@Override
	protected String getJobPath() throws Exception {
		URL jobUrl = new URL("platform:/plugin/fr.sanofi.fcl4transmart/jobs_kettle/load_metabolomic_annotation.kjb");
		jobUrl = FileLocator.toFileURL(jobUrl);  
		String jobPath = jobUrl.getPath();
		
		jobUrl = new URL("platform:/plugin/fr.sanofi.fcl4transmart/jobs_kettle/load_metabolomic_annotation_to_de_gpl_info.ktr");
		jobUrl = FileLocator.toFileURL(jobUrl); 
		jobUrl = new URL("platform:/plugin/fr.sanofi.fcl4transmart/jobs_kettle/load_metabolomic_annotation_to_lt.ktr");
		jobUrl = FileLocator.toFileURL(jobUrl); 
		jobUrl = new URL("platform:/plugin/fr.sanofi.fcl4transmart/jobs_kettle/run_i2b2_load_metabolomic_annot_deapp.ktr");
		jobUrl = FileLocator.toFileURL(jobUrl); 
	...
	protected String createUnixCommand() throws Exception {
		...
		command+="-file="+etlPreferences.getJobsDirectory()+"/load_metabolomic_annotation.kjb ";
