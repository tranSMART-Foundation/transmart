 ...
 */	
public class RnaSeqLoadAnnotationListener extends LoadAnnotationListener implements Listener{
	public RnaSeqLoadAnnotationListener(LoadAnnotationUI ui, DataTypeItf dataType){
		super(ui, dataType);
	}
	@Override
	protected void setParameters(){
		newPath=dataType.getPath().getAbsolutePath()+File.separator+platformId+"_annotation.txt";
		storedProcedureLaunched=".*Starting entry \\[i2b2_rna_seq_annotation\\].*";;
		storedProcedreEnded=".*Finished job entry \\[i2b2_rna_seq_annotation\\].*";
		queryStoredProcedureStarted="select max(JOB_ID) from CZ_JOB_AUDIT where STEP_DESC='Starting i2b2_rna_seq_annotation'";
		queryStoredProcedureEnded="select * from cz_job_audit where (step_desc like '%End i2b2_rna_seq_annotation%' or step_status='FAIL') and job_id=";
		unixDir="rnaSeq";
		checkPs=".*load_rnaseq_annotation.*";
	}
	@Override
	protected String getJobPath() throws Exception {
		...
		jobUrl = FileLocator.toFileURL(jobUrl); 
		jobUrl = new URL("platform:/plugin/fr.sanofi.fcl4transmart/jobs_kettle/load_rna_annotation_to_de_gpl_info.ktr");
		jobUrl = FileLocator.toFileURL(jobUrl); 
		jobUrl = new URL("platform:/plugin/fr.sanofi.fcl4transmart/jobs_kettle/load_all_rnaseq_annotation_to_lt.ktr");
		jobUrl = FileLocator.toFileURL(jobUrl); 
	...
	protected String createUnixCommand() throws Exception {
		...
		command+="-file="+etlPreferences.getJobsDirectory()+"/load_rnaseq_annotation.kjb ";
