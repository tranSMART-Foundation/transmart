...
import fr.sanofi.fcl4transmart.controllers.listeners.HDData.SelectRawFileListener;
...
public class SelectRawFilesUI implements WorkItf {
	...
	protected Listener removeFileListener;
	protected Listener addFileListener;
	...
	protected void setListeners(){
		...
		this.addFileListener=new SelectRawFileListener(this, this.dataType);
	}
	@Override
	public Composite createUI(Composite parent) {
		...
		add.addListener(SWT.Selection, this.addFileListener);
		...
		this.viewer.setInput(((HDDataItf)this.dataType).getRawFiles());
		gridData = new GridData();
		gridData.horizontalAlignment = SWT.FILL;
		gridData.grabExcessHorizontalSpace = true;
		...
		remove.addListener(SWT.Selection, this.removeFileListener);
	...
	public Vector<File> getSelectedRemovedFile(){
		...
		for(int i=0; i<paths.length; i++){
			if(((HDDataItf)this.dataType).getRawFilesNames().contains(paths[i])){
				files.add(new File(this.dataType.getPath()+File.separator+paths[i]));
			}
		}
		return files; 
	}
