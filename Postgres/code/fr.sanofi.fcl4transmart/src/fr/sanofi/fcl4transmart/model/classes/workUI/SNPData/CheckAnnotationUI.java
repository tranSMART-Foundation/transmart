		...
		new Thread(){
			public void run() {
				try{
					...
					tables.add("DEAPP.DE_SNP_PROBE");
					tables.add("DEAPP.DE_SNP_INFO");
					tables.add("DEAPP.DE_SNP_GENE_MAP");
					tables.add("DEAPP.DE_GPL_INFO");
					...
					//get the number of lines for de_gpl_info
					String gpl=FileHandler.getSnpGpl(((SnpData)dataType).getAnnotationProps());
					if(gpl.compareTo("")==0){
						expected.set(3, "0");
						inserted.set(3,"0");
						equals.set(3, "ERROR");
						...
						expected.set(3, "1");
						String n=String.valueOf(RetrieveData.getGplInfo(gpl));
						inserted.set(3,n);
						if(n.compareTo("1")==0){
							equals.set(3, "OK");
						}else{
					...
					//get the number of lines for de_snp_probe
					HashSet<String> probes=FileHandler.getProbesFromAnnotation(((SnpData)dataType).getAnnotationFile(), ((SnpData)dataType).getAnnotationProps());
					int nI=probes.size();
					int nE=RetrieveData.getSnpInfo(probes);
					...
					//get the number of lines for de_snp_info
					nI=probes.size();
					nE=RetrieveData.getSnpInfo(probes);
					...
					//get the number of lines for de_snp_gene_map
					HashSet<String> snpGenes=FileHandler.getProbesForGenesForSnp(dataType); 
					nI=snpGenes.size();
					nE=RetrieveData.getGeneMap(snpGenes);
