<html>
	...
	<div class="body">
		...
		<table>
			...
			<g:each in="${importXnatVariableList}" status='i' var='variable'>
				...
				<td class="actionButtons">
					...
					<g:link action='delete_coupling' id="${variable.id}"
						params="[configId: importXnatConfiguration.id]"
		...
		<div class="dialog">
			...
			<tbody>
				...
				<td valign="top" class="value ${hasErrors(bean:importXnatVariable,field:'datatype','errors')}">
				    <g:select name='datatype' from="${['Subject', 'Session', 'SubjectVariable', 'SessionVariable']}"
					      value="${fieldValue(bean:importXnatVariable, field='datatype')}"/>
				</td>
			    </tr>
		...
		<li>
		    XNAT (REST) URL: the REST api link to the attribute in XNAT. All possible links can be found here:
		    <a target="_blank" href="https://wiki.xnat.org/display/XNAT16/XNAT+REST+XML+Path+Shortcuts">XNAT REST XML Path Shortcuts</a><br/>
