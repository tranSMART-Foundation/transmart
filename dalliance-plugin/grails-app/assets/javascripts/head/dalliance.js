...
function loadDalliance(resultsTabPanel) {
    console.log('loadDalliance called');
	...
	console.log('loadDalliance readyState ' + nodReq.readyState);
        if (nodReq.readyState == 4) {
            if (nodReq.status == 200 || nodReq.status == 206) {
		console.log('loadDalliance status ' + nodReq.status + ' protocol ' + location.protocol + ' hostname ' + location.hostname + ' port ' + location.port);
