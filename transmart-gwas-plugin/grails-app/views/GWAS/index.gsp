...
<html>
	...
	<asset:stylesheet href="gwasTab.css"/>
	    ...
	    var gwasNewSearchURL = "${createLink(action: 'newSearch')}";
	    var gwasVisualizationURL = "${createLink(action: 'newVisualization')}"; %{-- not used --}%
	    var gwasTableURL = "${createLink(action: 'newTable')}"; %{-- not used --}%
	    var gwasTreeURL = "${createLink(action: 'getDynatree')}"; %{-- treeURL must overwrite transmartApp --}%
	    var gwasSourceURL = "${createLink(action: 'searchAutoComplete')}";
	    var gwasGetCategoriesURL = "${createLink(action: 'getSearchCategories')}";
	    ...
	    var getStudyAnalysesUrl = "${createLink(action: 'getTrialAnalysis')}";
        
	    //These are the URLS for the different browse windows.
	    var studyBrowseWindow = "${createLink(controller: 'experiment', action: 'browseGWASExperimentsMultiSelect')}";
	...
	<div id="search-div">         
	    ...
	    <input id="search-ac" placeholder="type search here"/>
	</div>
