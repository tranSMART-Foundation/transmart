    ...
    <g:each in="${experiments.entrySet().sort{it.key.accession}}" status='ti' var='experimentresult'>        
		...
		<span class="result-trial-name"> ${experimentresult.key.accession}</span>
		    ...
		    <a id="toggleFile_${experimentresult.key.id}" href="#"
		       onclick="javascript:gwasToggleFileDiv('${experimentresult.key.id}', '${createLink(controller:'fmFolder', action:'getFolderFiles', params:[id: experimentresult.key.id, trialNumber: experimentresult.key.id, unqKey: ts])}');">
		   ...
		   onclick="javascript:gwasToggleDetailDiv('${experimentresult.key.id}', '${createLink(controller:'GWAS', action:'getTrialAnalysis',params:[id: experimentresult.key.id, trialNumber: experimentresult.key.id, unqKey: ts])}');">
