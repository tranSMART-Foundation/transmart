...
<div id="table_results_length_div" class="dataTables_length">
	...
	<g:select name='resultCounter' size='1' from="${['100': '100', '150': '150', '200': '200', '500': '500']}"
		  ...
		  onchange="gwasLoadTableResultsGrid({max: jQuery(this).val()})" value="${max}"/> ${type} results
    </label>	
</div>
<g:if test="${showSearch}">
	...
	<label>Search: 
	    ...
	    <span class="linkbutton" onclick="gwasLoadTableResultsGrid({
...
<div class="dataTables_paginate paging_two_button">
    <g:if test="${offset > 0}">
	<a class="paginate_enabled_previous" tabindex="0" role="button" id="table_results_previous" aria-controls="results_table"
	   onclick="gwasLoadTableResultsGrid({offset: ${offset-max}})">Previous</a>
    </g:if>
    <g:else>
	...
	<a class="paginate_enabled_next" tabindex="0" role="button" id="table_results_next" aria-controls="results_table"
	   onclick="gwasLoadTableResultsGrid({offset: ${offset+max}})">Next</a>
    </g:if>
    <g:else>
