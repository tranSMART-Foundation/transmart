...
CREATE OR REPLACE FUNCTION tm_cz.i2b2_process_metabolomic_data(trial_id character varying, top_node character varying, data_type character varying DEFAULT 'R'::character varying, source_code character varying DEFAULT 'STD'::character varying, log_base numeric DEFAULT 2, secure_study character varying DEFAULT 'N'::character varying, currentjobid numeric DEFAULT 0) RETURNS numeric
    ...
    select category_cd,display_value,display_label,display_unit
      from tm_lz.lt_src_metabol_display_mapping;
...
begin
    ...
    if pCount = 0 then
	perform tm_cz.cz_write_audit(jobId,databasename,procedurename,'Platform not found in de_qpcr_metabolomics_annotation',1,stepCt,'ERROR');
	perform tm_cz.cz_error_handler(jobId, procedureName, SQLSTATE, SQLERRM);
	perform tm_cz.cz_end_audit (jobId,'FAIL');
	RETURN 163;
    end if;
    ...
    if pCount > 0 then
	perform tm_cz.cz_write_audit(jobId,databasename,procedurename,'Multiple platforms for sample_cd in lt_src_metabolomic_map',1,stepCt,'ERROR');
	perform tm_cz.cz_error_handler(jobId, procedureName, SQLSTATE, SQLERRM);
	perform tm_cz.cz_end_audit (jobId,'FAIL');
	RETURN 164;
    end if;
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert node values into wt_metabolomic_node_values',rowCt,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create leaf nodes in DEAPP tmp_metabolomics_nodes',rowCt,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create platform nodes in wt_metabolomic_nodes',rowCt,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create ATTR1 nodes in wt_metabolomic_nodes',rowCt,stepCt,'Done');
	...
	select
	    distinct topNode || regexp_replace(replace(replace(replace(replace(replace(replace(
		substr(category_cd,1,tm_cz.instr(category_cd,'ATTR2')+5),'PLATFORM',title),
		'ATTR1',coalesce(attribute_1,'')),
		'ATTR2',coalesce(attribute_2,'')),
		'TISSUETYPE',coalesce(tissue_type,'')),
		'+', '\'),
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create ATTR2 nodes in wt_metabolomic_nodes',rowCt,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create TISSUETYPE nodes in wt_metabolomic_nodes',rowCt,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Updated node_name in wt_metabolomic_nodes',rowCt,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert into wt_subject_mbolomics_probeset',rowCt,stepCt,'Done');
	...
	get diagnostics rowCt := ROW_COUNT;
	stepCt := stepCt + 1;
	perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert transformed into DEAPP DE_SUBJECT_METABOLOMICS_DATA',rowCt,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'End i2b2_process_metabolomic_data',0,stepCt,'Done');
