...
CREATE OR REPLACE FUNCTION tm_cz.i2b2_process_proteomics_data(trial_id character varying, top_node character varying, data_type character varying DEFAULT 'R'::character varying, source_cd character varying DEFAULT 'STD'::character varying, log_base numeric DEFAULT 2, secure_study character varying DEFAULT NULL::character varying, currentjobid numeric DEFAULT NULL::numeric) RETURNS numeric
    ...
    /*************************************************************************
     * This storeD procedure is for ETL for Sanofi to load  proteomics data
     * Date: 9/12/2013
     ******************************************************************/
    --	***  NOTE ***
    --	***  NOTE ***
...
begin
    ...
    stepCt := stepCt + 1;
    get diagnostics rowCt := ROW_COUNT;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Updated node_name in DEAPP tmp_proteomics_nodes',rowCt,stepCt,'Done');
	...
	get diagnostics rowCt := ROW_COUNT;
	if rtnCd > 1 then
	    perform tm_cz.cz_write_audit(jobId,databasename,procedurename,'Error while executing tm_cz.i2b2_add_node(' || TrialID || ',' || r_addNodes.leaf_node || ',' || r_addNodes.node_name || ',' || jobId || ')' ,1,stepCt,'ERROR');
	    perform tm_cz.cz_end_audit (jobId,'FAIL');
	    return 168;
	end if;
	  ...
	  from (select
		  ...
		  from tm_lz.lt_src_proteomics_sub_sam_map a
		    --Joining to Pat_dim to ensure the ID's match. If not I2B2 won't work.
			   ...
			   left outer join tm_wz.wt_proteomics_nodes pn
    ...
    if pExists = 0 then
	stepCt := stepCt + 1;
	perform tm_cz.cz_write_audit(jobId,databasename,procedurename,'Unable to match probesets to platform in probeset_deapp',1,stepCt,'ERROR');
	perform tm_cz.cz_ERROR_HANDLER(JOBID,PROCEDURENAME, '-1', 'Application raised error');
	perform tm_cz.cz_end_audit (jobId,'FAIL');
	return 165;
    end if;
	...
	stepCt := stepCt + 1;
	get diagnostics rowCt := ROW_COUNT;
	perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert transformed into de_subject_protein_data',rowCt,stepCt,'Done');
    else
    ...
    stepCt := stepCt + 1;
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'End i2b2_process_proteomics_data',0,stepCt,'Done');
