...
begin
    ...
    perform tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Start i2b2_load_clinical_inc_data ',0,stepCt,'Done');
    ...
    */	
	     ...
	     where n.c_fullname in  
		   (select leaf_node from wt_trial_nodes
		     where (((category_cd||'+'||replace(data_label,'PCT','%'))||node_name)=(ul.category_cd||ul.display_label) 
			    or (category_cd||node_name)=(ul.category_cd||ul.display_label))
	...
	delete from i2b2demodata.observation_fact f
	 where f.sourcesystem_cd = TrialId
               and (f.patient_num||f.concept_cd) in
	...
	with upd as (select p.c_fullname, count(*) as nbr_children 
		...
		set c_visualattributes=case when upd.nbr_children = 1 
		    then 'L' || substr(b.c_visualattributes,2,2)
		        ...
		        else substr(b.c_visualattributes,3,1)
		        end
		    end
