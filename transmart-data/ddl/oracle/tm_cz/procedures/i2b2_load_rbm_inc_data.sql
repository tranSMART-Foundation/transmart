...
AS
	...
	select category_cd,display_value,display_label,display_unit
	  from tm_lz.lt_src_rbm_display_mapping;
...
BEGIN
    ...
    stepCt := 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_load_rbm_data',0,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Uppercase trial_name in LT_SRC_RBM_SUBJ_SAMP_MAP',SQL%ROWCOUNT,stepCt,'Done');
    commit;

    ---Data correction
    update tm_lz.lt_src_rbm_data m set m.avalue=replace(m.avalue,'<LOW>','0');
    update tm_lz.lt_src_rbm_data m set m.avalue=to_char(to_number(substr(m.avalue,instr(m.avalue,'>')+1,length(m.avalue)))+1);
    update tm_lz.lt_src_rbm_data m set m.avalue=to_char(to_number(substr(m.avalue,instr(m.avalue,'<')+1,length(m.avalue)))-1);

    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Data correction in LT_SRC_RBM_DATA',SQL%ROWCOUNT,stepCt,'Done');
    commit;
     ...
     where obf.concept_cd in
	   (select distinct x.concept_code
	     ...
	     where x.trial_name = TrialId
	       ...
	       and x.platform = 'RBM'
               and (x.subject_id||x.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_rbm_subj_samp_map));
    ...
    delete from deapp.de_subject_sample_mapping
     where trial_name = TrialID
	   ...
	   and platform = 'RBM'
	   and (subject_id||sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_rbm_subj_samp_map); --Making sure only rbm data is deleted
	...
	--and patient_id in (select p.patient_num from i2b2demodata.patient_dimension p,LT_SRC_RBM_SUBJ_SAMP_MAP s
        -- where p.sourcesystem_cd=regexp_replace(TrialID || ':' || s.site_id || ':' || s.subject_id,'(::){1,}', ':'))
           ;
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Delete data from de_subject_rbm_data for  updation',SQL%ROWCOUNT,stepCt,'Done');
    commit;
	...
	--where trial_name = TrialId ;
	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Delete data from de_subject_rbm_data',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    else
	...
	if pExists = 0 then
	    ...
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Adding partition to de_subject_rbm_data',0,stepCt,'Done');

	else
	    sqlText := 'alter table deapp.de_subject_rbm_data truncate partition "' || TrialID || ':' || sourceCd || '"';
	    execute immediate(sqlText);
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Truncating partition in de_subject_rbm_data',0,stepCt,'Done');
	end if;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert node values into DEAPP wt_rbm_node_values',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create leaf nodes in DEAPP tmp_rbm_nodes',SQL%ROWCOUNT,stepCt,'Done');
    commit;
	...
	,'PLATFORM'
      from tm_wz.wt_rbm_nodes;

    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create platform nodes in wt_rbm_nodes',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create ATTR1 nodes in WT_RBM_NODES',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create ATTR2 nodes in WT_RBM_NODES',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create ATTR2 nodes in wt_qpcr_rbm_nodes',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Updated node_name in DEAPP tmp_rbm_nodes',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Update WT_RBM_NODES with newly created concept_cds',SQL%ROWCOUNT,stepCt,'Done');
    commit;
	...
	,'' -- no units available
     ...
     where m.trial_name = TrialID
       ...
       and m.platform = 'RBM'
       and (m.subject_id||m.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_rbm_subj_samp_map);
    ...
    insert into tm_wz.wt_subject_rbm_probeset (
	--mod
	probeset
	--	,expr_id
	,intensity_value
	...
	--,tm_lz.lt_src_rbm_annotation  gs
	...
	--and trim(substr(md.analyte,1,instr(md.analyte,'(')-1)) =trim(gs.antigen_name)
       ...
       and (sd.subject_id||sd.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_rbm_subj_samp_map)
     group by md.analyte
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert into DEAPP wt_subject_rbm_probeset',SQL%ROWCOUNT,stepCt,'Done');
	...
	TrialId
        ...
        --	  ,m.sample_id
	--	  ,m.subject_id
	from tm_wz.wt_subject_rbm_probeset m
	...
	-- and p.probeset=m.probeset_id
        ;
	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert transformed into DEAPP de_subject_rbm_data',SQL%ROWCOUNT,stepCt,'Done');
	...
	if dataType = 'R' or dataType = 'L' then
	    tm_cz.i2b2_rbm_inc_zscore_calc(TrialID,'L',jobId,dataType,logBase,sourceCD);
	    stepCt := stepCt + 1;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'End i2b2_LOAD_RBM_DATA',0,stepCt,'Done');
...
EXCEPTION
	...
	tm_cz.cz_write_audit(jobId,databasename,procedurename,'Platform not found in de_rbm_annotation',1,stepCt,'ERROR');
	tm_cz.cz_error_handler(jobId,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
	select 163 into rtn_code from dual;--mod
    when multiple_platform then
	tm_cz.cz_write_audit(jobId,databasename,procedurename,'Multiple platforms for sample_cd in LT_SRC_RBM_SUBJ_SAMP_MAP',1,stepCt,'ERROR');
	tm_cz.cz_error_handler(jobId,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
