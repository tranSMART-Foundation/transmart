...
BEGIN
    ...
    stepCt := 0;
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Starting I2B2_MRNA_INC_SUB_ZSCORE',0,stepCt,'Done');

    -- call the cursor and update the z-score value for incmental data;
    for UpdateZscore in zscore_params
        loop

        update deapp.de_subject_microarray_data d
    ...
    update deapp.de_subject_microarray_data
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'END of I2B2_MRNA_INC_SUB_ZSCORE',0,stepCt,'Done');
    commit;
