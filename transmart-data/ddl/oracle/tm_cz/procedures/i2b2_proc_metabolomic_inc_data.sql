...
BEGIN
	   ...
	   (select distinct x.concept_code
	     ...
	     where x.trial_name = TrialId
	       ...
	       and x.platform = 'METABOLOMICS'
               and (x.subject_id||x.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_metabolomic_map));
    ...
    delete from deapp.de_subject_sample_mapping
     where trial_name = TrialID
	   ...
	   and (subject_id||sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_metabolomic_map); --Making sure only metabolomic data is deleted
	...
	--and patient_id in (select p.patient_num from i2b2demodata.patient_dimension p,LT_SRC_METABOLOMIC_MAP s
	--where p.sourcesystem_cd=regexp_replace(TrialID || ':' || s.site_id || ':' || s.subject_id,'(::){1,}', ':'))
	   ;
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Delete data from DE_SUBJECT_METABOLOMICS_DATA for  updation',SQL%ROWCOUNT,stepCt,'Done');
    commit;
	...
	,'' -- no units available
     ...
     where m.trial_name = TrialID
       ...
       and (m.subject_id||m.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_metabolomic_map);
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'insert distinct sample_cd in i2b2demodata.sample_dimension from deapp.de_subject_sample_mapping',SQL%ROWCOUNT,stepCt,'Done');
    commit;
	...
	--  and md.peptide =p.peptide-- gs.mirna_id
       ...
       and (sd.subject_id||sd.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_metabolomic_map)
	...
	if dataType = 'R' or dataType = 'L' then
	    tm_cz.i2b2_metbolo_inc_zscore_calc(TrialID,'L',jobId,dataType,logBase,sourceCD);
	    stepCt := stepCt + 1;
