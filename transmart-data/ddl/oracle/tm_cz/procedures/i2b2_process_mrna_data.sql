...
AS
	...
	select category_cd,display_value,display_label,display_unit
	  from tm_lz.lt_src_display_mapping group by category_cd,display_value,display_label,display_unit;
...
EXCEPTION
	...
	tm_cz.cz_error_handler(jobid,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
	select 161 into rtnCd from dual;
	...
	tm_cz.cz_error_handler(jobid,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
	select 162 into rtnCd from dual;
	...
	tm_cz.cz_error_handler(jobId,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
	select 169 into rtnCd from dual;
	...
	tm_cz.cz_error_handler(jobId,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
	select 163 into rtnCd from dual;
    when no_probeset_recs then
	tm_cz.cz_write_audit(jobId,databasename,procedurename,'Unable to match probesets to platform in de_mrna_annotation',1,stepCt,'ERROR');
	tm_cz.cz_error_handler(jobId,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
	select 165 into rtnCd from dual;
    WHEN OTHERS THEN
	--Handle errors.
	tm_cz.cz_error_handler (jobID, procedureName);
    --End Proc
	tm_cz.cz_end_audit (jobID, 'FAIL');
	select 166 into rtnCd from dual;
