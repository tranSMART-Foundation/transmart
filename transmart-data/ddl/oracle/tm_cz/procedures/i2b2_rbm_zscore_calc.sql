...
BEGIN
	...
	if stgTrial != TrialId then
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'TrialId not the same as trial in WT_SUBJECT_RBM_PROBESET - procedure exiting',SQL%ROWCOUNT,stepCt,'Done');
	...
	if idxExists = 0 then
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'No data for TrialId in de_subject_rbm_data - procedure exiting',SQL%ROWCOUNT,stepCt,'Done');
	    raise trial_missing;
	end if;
    end if;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Loaded data for trial in TM_WZ wt_subject_rbm_logs',SQL%ROWCOUNT,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create index on TM_WZ wt_subject_rbm_logs',0,stepCt,'Done');
	...
	,stddev(log_intensity)
     ...
     group by d.trial_name
	      ,d.probeset_id;
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Calculate intensities for trial in TM_WZ wt_subject_rbm_calcs',SQL%ROWCOUNT,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create index on TM_WZ wt_subject_rbm_calcs',0,stepCt,'Done');

    -- calculate zscore

    insert into tm_wz.wt_subject_rbm_med parallel (
	probeset_id
	,intensity_value
	,log_intensity
	,assay_id
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Calculate Z-Score for trial in TM_WZ wt_subject_rbm_med',SQL%ROWCOUNT,stepCt,'Done');
    ...
    /*
      ...
      from tm_wz.wt_subject_rbm_med;

      if nbrRecs > 10000000 then
      tm_cz.i2b2_mrna_index_maint('DROP',,jobId);
      stepCt := stepCt + 1;
      tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Drop indexes on DEAPP de_subject_rbm_data',0,stepCt,'Done');
      else
      stepCt := stepCt + 1;
	...
	--	  ,m.subject_id
      from tm_wz.wt_subject_rbm_med m
           ...
           ,(select distinct antigen_name, gpl_id,gene_symbol,gene_id from deapp.de_rbm_annotation ) a
    ...
    select d.id, ann.id
     ...
     where not exists( select * from deapp.de_rbm_data_annotation_join j
			where j.data_id = d.id AND j.annotation_id = ann.id );
    /*
      ...
      --	  ,m.sample_id
      --	  ,m.subject_id
      from tm_wz.wt_subject_rbm_med m, deapp.de_rbm_annotation a,deapp.de_subject_sample_mapping d
      --(select distinct intensity_value from tm_wz.wt_subject_rbm_probeset p) x
     ...
     */
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert data for trial in DEAPP de_subject_rbm_data',SQL%ROWCOUNT,stepCt,'Done');
    ...
    /*
      ...
      stepCt := stepCt + 1;
      tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Add indexes on DEAPP de_subject_rbm_data',0,stepCt,'Done');
     */
