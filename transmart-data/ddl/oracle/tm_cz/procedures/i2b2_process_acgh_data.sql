...
AS
	...
	select distinct c_fullname
	 ...
	 where c_fullname like topNode || '%'  and
...
BEGIN
    ...
    stepCt := 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_process_acgh_data',0,stepCt,'Done');

    -- Get count of records in lt_src_mrna_subj_samp_map

    select count(*) into sCount from tm_lz.lt_src_mrna_subj_samp_map;

    -- check if all subject_sample map records have a platform, If not, abort run

    select count(*) into pCount
      from tm_lz.lt_src_mrna_subj_samp_map where platform is null;
	...
	delete from deapp.de_subject_acgh_data
	where trial_source = TrialId || ':' || sourceCd;
	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Delete data from de_subject_acgh_data',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    else
	...
	if pExists = 0 then
	    ...
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Adding partition to de_subject_acgh_data',0,stepCt,'Done');
	else
	    sqlText := 'alter table deapp.de_subject_acgh_data truncate partition "' || TrialID || '"';
	    execute immediate(sqlText);
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Truncating partition in de_subject_acgh_data',0,stepCt,'Done');
	end if;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert into DEAPP de_subject_acgh_region',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'End i2b2_process_acgh_data',0,stepCt,'Done');
