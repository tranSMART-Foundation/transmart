...
BEGIN
    ...
    execute immediate 'delete from biomart.bio_data_uid where unique_id in
                      (select tm_cz.biomarker_pathway_uid(primary_source_code, primary_external_id)
                      from biomart.bio_marker)';
    execute immediate 'delete from biomart.bio_data_uid where unique_id in
                      (select tm_cz.biomarker_gene_uid(primary_external_id)
                      from biomart.bio_marker)';
    execute immediate 'insert into biomart.bio_data_uid(
                      bio_data_id, unique_id, bio_data_type)
                      select
                      bio_marker_id
                      , tm_cz.biomarker_pathway_uid(primary_source_code, primary_external_id)
                      , ''BIO_MARKER.PATHWAY''
					     from biomart.bio_marker
					     where bio_marker_type=''PATHWAY''
									     and not exists
									     (select 1 from biomart.bio_data_uid
									     where tm_cz.biomarker_pathway_uid(bio_marker.primary_source_code, bio_marker.primary_external_id) =
									     bio_data_uid.unique_id)';
    execute immediate 'insert into biomart.bio_data_uid(
                      bio_data_id, unique_id, bio_data_type)
                      select
                      bio_marker_id
                      , tm_cz.biomarker_gene_uid(primary_external_id)
                      , ''BIO_MARKER.GENE''
					  from biomart.bio_marker
					  where bio_marker_type=''GENE''
								       and not exists
								       (select 1 from biomart.bio_data_uid
								       where tm_cz.biomarker_gene_uid(bio_marker.primary_external_id) = bio_data_uid.unique_id)';
