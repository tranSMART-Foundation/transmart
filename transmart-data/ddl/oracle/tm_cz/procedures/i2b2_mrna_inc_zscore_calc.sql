...
BEGIN
	...
	if stgTrial != TrialId then
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'TrialId not the same as trial in wt_subject_mrna_probeset - procedure exiting',SQL%ROWCOUNT,stepCt,'Done');
	...
	if idxExists = 0 then
	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'No data for TrialId in de_subject_microarray_data - procedure exiting',SQL%ROWCOUNT,stepCt,'Done');
	raise trial_missing;
	end if;
	end if;
     */
...
--	,subject_id
    ...
    select count(*) into pExists from deapp.de_subject_microarray_data where trial_name=TrialId;
    ...
    if pExists > 0 then
	tm_cz.i2b2_mrna_inc_sub_zscore(TrialId,jobId);

	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'update zscore in de_subject_microarray_data ',0,stepCt,'Done');
    end if;
    ...
    /*
      ...
      stepCt := stepCt + 1;
      tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Add indexes on DEAPP de_subject_microarray_data',0,stepCt,'Done');
     */
