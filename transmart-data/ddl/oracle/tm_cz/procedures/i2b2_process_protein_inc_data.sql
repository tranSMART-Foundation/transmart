...
AS
	...
	select category_cd,display_value,display_label,display_unit
	  from tm_lz.lt_src_protein_display_mapping;
...
BEGIN
    ...
    stepCt := 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_process_proteomics_data',0,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Uppercase trial_name in LT_SRC_PROTEOMICS_SUB_SAM_MAP',SQL%ROWCOUNT,stepCt,'Done');
    commit;
	   ...
	   (select distinct x.concept_code
	     ...
	     where x.trial_name = TrialId
	       ...
	       and x.platform = 'PROTEIN'
               and (x.subject_id||x.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_proteomics_sub_sam_map));
    ...
    delete from deapp.de_subject_sample_mapping
     where trial_name = TrialID
	   ...
	   and (subject_id||sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_proteomics_sub_sam_map); --Making sure only miRNA data is deleted
	...
	--and patient_id in (select p.patient_num from i2b2demodata.patient_dimension p,tm_lz.lt_src_proteomics_sub_sam_map s where
        -- p.sourcesystem_cd=regexp_replace(TrialID || ':' || s.site_id || ':' || s.subject_id,'(::){1,}', ':'))
           ;

    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Delete data from DE_SUBJECT_PROTEIN_DATA for  updation',SQL%ROWCOUNT,stepCt,'Done');
    commit;
	...
	--where trial_name = TrialId ;
	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Delete data from DE_SUBJECT_PROTEIN_DATA',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    else
	...
	if pExists = 0 then
	    ...
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Adding partition to DE_SUBJECT_PROTEIN_DATA',0,stepCt,'Done');

	else
	    sqlText := 'alter table deapp.de_subject_protein_data truncate partition "' || TrialID || ':' || sourceCd || '"';
	    execute immediate(sqlText);
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Truncating partition in DE_SUBJECT_PROTEIN_DATA',0,stepCt,'Done');
	end if;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert node values into DEAPP WT_PROTEOMICS_NODE_VALUES',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create leaf nodes in DEAPP tmp_proteomics_nodes',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create platform nodes in WT_PROTEOMICS_NODES',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create ATTR1 nodes in WT_PROTEOMICS_NODES',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Create ATTR2 nodes in WT_PROTEOMICS_NODES',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Updated node_name in DEAPP tmp_proteomics_nodes',SQL%ROWCOUNT,stepCt,'Done');
    commit;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Update WT_PROTEOMICS_NODES with newly created concept_cds',SQL%ROWCOUNT,stepCt,'Done');
    commit;
	...
	,'' -- no units available
     ...
     where m.trial_name = TrialID
       ...
       and (m.subject_id||m.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_proteomics_sub_sam_map);
    ...
    insert into tm_wz.wt_subject_proteomics_probeset (
	--mod
	probeset
	--	,expr_id
	,intensity_value
	...
	--  and md.peptide =p.peptide-- gs.mirna_id
       ...
       and (sd.subject_id||sd.sample_cd) in (select (subject_id||sample_cd) from tm_lz.lt_src_proteomics_sub_sam_map)
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert into DEAPP WT_SUBJECT_PROTEOMICS_PROBESET',SQL%ROWCOUNT,stepCt,'Done');
	...
	/*
	  ...
	  update deapp.de_subject_protein_data dm set (dm.protein_annotation_id
	  ,dm.component
	  ,dm.gene_symbol
	  ,dm.gene_id
	  ,dm.assay_id
	  ...
	  --,deapp.de_subject_protein_data de
	  ...
	  stepCt := stepCt + 1;
	  tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Update into DEAPP de_subject_protein_data',SQL%ROWCOUNT,stepCt,'Done');
	...
	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert transformed into DEAPP DE_SUBJECT_PROTEIN_DATA',SQL%ROWCOUNT,stepCt,'Done');
	...
	if dataType = 'R' or dataType = 'L' then
	    tm_cz.i2b2_protein_inc_zscore_calc(TrialID,'L',jobId,dataType,logBase,sourceCD);
	    stepCt := stepCt + 1;
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'End i2b2_process_proteomics_data',0,stepCt,'Done');
...
EXCEPTION
	...
	tm_cz.cz_write_audit(jobId,databasename,procedurename,'Platform not found in de_qpcr_proteomics_annotation',1,stepCt,'ERROR');
	tm_cz.cz_error_handler(jobId,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
	select 163 into rtn_code from dual;--mod
    when multiple_platform then
	tm_cz.cz_write_audit(jobId,databasename,procedurename,'Multiple platforms for sample_cd in LT_SRC_PROTEOMICS_SUB_SAM_MAP',1,stepCt,'ERROR');
	tm_cz.cz_error_handler(jobId,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
