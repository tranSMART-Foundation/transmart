...
AS
	...
	select distinct t.leaf_node,t.node_name
	 ...
	 where not exists (select 1 from i2b2metadata.i2b2 x where t.leaf_node = x.c_fullname);
...
BEGIN
    ...
    stepCt := 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_process_rnaseq_data',0,stepCt,'Done');
    ...
    select count(*) into pCount
     ...
     where gpl_id in (select distinct m.platform
			from tm_lz.lt_src_mrna_subj_samp_map m);
    ...
    select count(*) into pCount
     ...
     where platform in (select distinct m.platform
			  from tm_lz.lt_src_mrna_subj_samp_map m);
    ...
    select count(*) into pExists
      from i2b2metadata.table_access where c_name = rootNode;
    ...
    select c_hlevel into root_level
      from i2b2metadata.i2b2 where c_name = RootNode;
	...
	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Delete data from de_subject_rnaseq_data',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    else
	...
	if pExists = 0 then
	    ...
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Adding partition to de_subject_rnaseq_data',0,stepCt,'Done');
	else
	    sqlText := 'alter table deapp.de_subject_rnaseq_data truncate partition "' || TrialID || '"';
	    execute immediate(sqlText);
	    stepCt := stepCt + 1;
	    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Truncating partition in de_subject_rnaseq_data',0,stepCt,'Done');
	end if;
    ...
    update tm_wz.wt_mrna_nodes t
       set concept_cd=(select c.concept_cd
    ...
    select distinct sample_cd
     ...
     where sample_cd not in (select sample_cd
    ...
    update i2b2metadata.i2b2 t
     ...
     where t.c_basecode in (select distinct x.concept_cd
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Insert into DEAPP wt_subject_rnasq_region',SQL%ROWCOUNT,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'End i2b2_process_rnaseq_data',0,stepCt,'Done');
...
EXCEPTION
	...
	tm_cz.cz_write_audit(jobId,databasename,procedurename,'Platform not found in de_chromosomal_region',1,stepCt,'ERROR');
	tm_cz.cz_error_handler(jobId,procedurename);
	tm_cz.cz_end_audit (jobId,'FAIL');
