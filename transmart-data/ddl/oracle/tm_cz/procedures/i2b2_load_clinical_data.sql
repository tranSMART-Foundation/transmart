...
BEGIN
    ...
    if pCount > 2 then
	stepCt := stepCt + 1;
	tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Adding upper-level nodes',0,stepCt,'Done');
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Set visit_name to null when only DATALABE in category_cd',SQL%ROWCOUNT,stepCt,'Done');
    ...
    select count(*) into pExists from tm_lz.lt_src_subj_enroll_date
     where enroll_date is not null and tm_cz.is_date(enroll_date,'YYYY/MM/DD HH24:mi') = 1;
    stepCt := stepCt + 1;
    ...
    insert into tm_cz.tmp_subject_info (
	usubjid,
	age_in_years_num,
	sex_cd,
	race_cd
    ...
    delete i2b2demodata.patient_dimension
    ...
    delete /*+ parallel(observation_fact, 4) */ from i2b2demodata.observation_fact F
     where f.modifier_cd = TrialId
	   ...
	   (select /*+ parallel(de_subject_sample_mapping, 4) */ distinct concept_code as concept_cd from deapp.de_subject_sample_mapping
	     where trial_name = TrialId
	       and concept_code is not null
	     union
	    select /*+ parallel(de_subject_sample_mapping, 4) */ distinct platform_cd as concept_cd from deapp.de_subject_sample_mapping
	     where trial_name = TrialId
	       and platform_cd is not null
	     union
	    select /*+ parallel(de_subject_sample_mapping, 4) */ distinct sample_type_cd as concept_cd from deapp.de_subject_sample_mapping
	     where trial_name = TrialId
	       and sample_type_cd is not null
	     union
	    select /*+ parallel(de_subject_sample_mapping, 4) */ distinct tissue_type_cd as concept_cd from deapp.de_subject_sample_mapping
	     where trial_name = TrialId
	       and tissue_type_cd is not null
	     union
	    select /*+ parallel(de_subject_sample_mapping, 4) */ distinct timepoint_cd as concept_cd from deapp.de_subject_sample_mapping
	     where trial_name = TrialId
	       and timepoint_cd is not null
	     union
	    select /*+ parallel(de_subject_sample_mapping, 4) */ distinct concept_cd as concept_cd from deapp.de_subject_snp_dataset
	     where trial_name = TrialId
    ...
    stepCt := stepCt + 1;
    tm_cz.cz_write_audit(jobId,databaseName,procedureName,'End i2b2_load_clinical_data',0,stepCt,'Done');
