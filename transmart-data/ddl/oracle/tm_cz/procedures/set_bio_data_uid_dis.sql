...
begin
    ...
    execute immediate 'delete from biomart.bio_data_uid where unique_id in
                      (select tm_cz.bio_disease_uid(mesh_code) from biomart.bio_disease)';
    execute immediate 'insert into biomart.bio_data_uid(
                      bio_data_id, unique_id, bio_data_type)
                      select
                      bio_disease_id, tm_cz.bio_disease_uid(mesh_code), ''BIO_DISEASE''
										      from biomart.bio_disease
										      where not exists
										      (select 1 from biomart.bio_data_uid
										      where tm_cz.bio_disease_uid(bio_disease.mesh_code) = bio_data_uid.unique_id)';
