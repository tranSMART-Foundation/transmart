...
class CorrelatedBiomarkersDataConstraint implements CriteriaDataConstraint {
    ...
    // TODO use DI
    private SessionFactory sessionFactory = Holders.applicationContext.sessionFactory
    ...
    class CorrelatedBiomarkersCriterion extends SQLCriterion {
		...
		throw new HibernateException('Could not find Criteria with alias ' + outer.entityAlias +
            ...
            if (entityName == null) {
		throw new HibernateException('Could not find entity name for criteria ' + relevantCriteria +
					     '. Map of criteria entity names is ' + relevantCriteria.criteriaEntityNames)
            }

            // Yikes!
	    String propertyColumn = outer.sessionFactory.getClassMetadata(entityName).getPropertyColumnNames(outer.propertyToRestrict)[0]
            String sqlAlias = criteriaQuery.getSQLAlias(relevantCriteria)
	    toString().replaceAll(/\{alias\}/, sqlAlias).replaceAll(/\{property\}/, propertyColumn)
        }
    }
}
