...
class ConceptKey {
    ...
    ConceptKey(String key) {
        ...
        def matcher = (key =~ '\\\\')
        if (!matcher.find(3)) {
            throw new IllegalArgumentException('No \\ character found after position 2')
	}
    ...
    ConceptKey(String tableCode, String fullName) {
        if (tableCode == null || fullName == null) {
            throw new IllegalArgumentException('Table code or full name are null')
	}
        if (tableCode.find('\\\\') != null) {
            throw new IllegalArgumentException('Table code includes a back slash')
	}
