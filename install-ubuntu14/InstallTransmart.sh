...
if ! [ -e transmart-data-release-16.2.zip ] ; then
	curl https://codeload.github.com/tranSMART-Foundation/transmart-data/zip/release-16.2 -o transmart-data-release-16.2.zip
fi
...
if ! [ -e transmart.war ]; then
	curl http://library.transmartfoundation.org/wars/transmart.V16.2.war --output transmart.war
fi
if ! [ -e gwava.war ]; then
	curl http://library.transmartfoundation.org/wars/gwava.V16.2.war --output gwava.war
fi
