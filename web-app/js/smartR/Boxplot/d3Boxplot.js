...
var SmartRBoxplot = (function () {
    ...
    service.create = function (data) {
        ...
        var results = data; // TODO reading from JSON response
            ...
            if (!checked) {
                ...
                for (var j = 0; j < results.cohort1.subsets.length; j++) {
                    d3.selectAll('.point.cohort1.' + shortenNodeLabel(results.cohort1.subsets[j]))
                        ...
                        .duration(animationDuration)
                        .attr("cx", function (d) {
                            return x1(results.cohort1.subsets[j]) + boxplotWidth * jitterWidth * d.jitter;
                        });
                }
                for (j = 0; j < results.cohort2.subsets.length; j++) {
                    d3.selectAll('.point.cohort2.' + shortenNodeLabel(results.cohort2.subsets[j]))
                        ...
                        .duration(animationDuration)
                        .attr("cx", function (d) {
                            return x2(results.cohort2.subsets[j]) + boxplotWidth * jitterWidth * d.jitter;
                        });
                }
                jitterChecked = true;
            }
        ...
        function createBoxplot(params, cohort, subset, x, y) {
            ...
            var whisker = boxplots[cohort][subset].selectAll('.whisker')
                .data([params.upperWhisker, params.lowerWhisker], function (d, i) {
                    return cohort + '-' + shortenedSubset + '-whisker-' + i;
                });
            whisker.enter()
                .append('line')
                .attr('class', 'whisker');
            ...
            var whiskerLabel = boxplots[cohort][subset].selectAll('.whiskerLabel')
                .data([params.upperWhisker, params.lowerWhisker], function (d, i) {
                    return cohort + '-' + shortenedSubset + '-whiskerLabel-' + i;
                });
            whiskerLabel
                .enter()
                .append('text')
            ...
            var hinge = boxplots[cohort][subset].selectAll('.hinge')
                .data([params.upperHinge, params.lowerHinge], function (d, i) {
                    return cohort + '-' + shortenedSubset + '-hinge-' + i;
                });
            hinge.enter()
                .append('line')
                .attr('class', 'hinge');
            ...
            var hingeLabel = boxplots[cohort][subset].selectAll('.hingeLabel')
                .data([params.upperHinge, params.lowerHinge], function (d, i) {
                    return cohort + '-' + shortenedSubset + '-hingeLabel-' + i;
                });
            hingeLabel.enter()
                .append('text')
            ...
            var connection = boxplots[cohort][subset].selectAll('.connection')
                ...
                function (d, i) {
                    return cohort + '-' + shortenedSubset + '-connection-' + i;
                });
            connection.enter()
                .append('line')
            ...
            var upperBox = boxplots[cohort][subset].selectAll('.box.upper')
                .data(params.upperHinge, function (d) {
                    return cohort + '-' + shortenedSubset + '-upperBox';
                });
            upperBox.enter()
                .append('rect')
            ...
            var lowerBox = boxplots[cohort][subset].selectAll('.box.lower')
                .data(params.lowerHinge, function (d) {
                    return cohort + '-' + shortenedSubset + '-lowerBox';
                });
            lowerBox.enter()
                .append('rect')
            ...
            var medianLabel = boxplots[cohort][subset].selectAll('.medianLabel')
                .data(params.median, function (d) {
                    return cohort + '-' + shortenedSubset + '-medianLabel';
                });
            medianLabel.enter()
                .append('text')
            ...
            var point = boxplots[cohort][subset].selectAll('.point')
                .data(params.points, function (d) {
                    return cohort + '-' + shortenedSubset + '-' + d.patientID;
                });
            point
                .enter()
                .append('circle')
                .attr("cx", function (d) {
                    return jitterChecked ? x(subset) + boxplotWidth * jitterWidth * d.jitter : x(subset);
                })
                .attr("r", 0)
            ...
            var values = params.points.map(function (d) {
                return d.value;
            });
            var estFun = kde(values);
            var kdeDomain = d3.extent(estFun, function (d) {
                return d[1];
            });
            ...
            var lineGen = d3.svg.line()
                .x(function (d) {
                    return x(subset) - kdeScale(d[1]);
                })
                .y(function (d) {
                    return y(d[0]);
                });
            var line = boxplots[cohort][subset].selectAll('.line')
                .data([cohort], function (d) {
                    return cohort;
                });
            line.enter()
                .append("path")
