...
connectToTransmart <- 
function (transmartDomain, use.authentication = TRUE, access.token = NULL, refresh.token = NULL, ...) {
    ...
    if (!is.null(refresh.token)) {
        transmartClientEnv$refresh_token <- refresh.token
    }
