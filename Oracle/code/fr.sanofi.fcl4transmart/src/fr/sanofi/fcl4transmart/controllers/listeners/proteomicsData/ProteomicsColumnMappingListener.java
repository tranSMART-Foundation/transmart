...
public class ProteomicsColumnMappingListener implements Listener {
	private DataTypeItf dataType;
	private ProteomicsSetColumnMappingUI ui;
	private Vector<Integer> peptideIdNum;
	private Vector<Integer> valueStartNum;
	private Vector<Integer> valueEndNum;
	public ProteomicsColumnMappingListener (DataTypeItf dataType, ProteomicsSetColumnMappingUI ui){
	...
	public void handleEvent(Event event) {
		...
		try{			  
			...
			File columnMapping=((ProteomicsData)this.dataType).getColumnMappingFile();
			for(int i=0; i<size; i++){
			...
			try{
				...
				((ProteomicsData)this.dataType).setColumnMappingFile(fileDest);
			}
