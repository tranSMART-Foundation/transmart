...
import fr.sanofi.fcl4transmart.controllers.listeners.metabolomicsData.QCListener;
...
/**
 *This class allows the creation of the composite for metabolomics data quality control
 */
public class MetabolomicsQualityControlUI implements WorkItf{
	...
	public MetabolomicsQualityControlUI(DataTypeItf dataType){
	...
	public Composite createUI(Composite parent){
		...
		new Thread(){
			public void run() {
				number=String.valueOf(RetrieveData.getMetabolomicsProbeNumber(dataType.getStudy().toString()));
		...
		transcriptNumber.setText("Biochemical number: "+this.number);
		...
		transcriptLabel.setText("Biochemical identifier: ");
	...
	public Composite createBody(String transcript){
		...
		for(File rawFile: ((MetabolomicsData)this.dataType).getRawFiles()){
			transcripts.addAll(FileHandler.getPeptide(rawFile, ((MetabolomicsData)this.dataType).getColumnMappingFile()));
		}
		if(!transcripts.contains(transcriptId)){
			Label label=new Label(body, SWT.NONE);
			label.setText("This biochemical identifier does not exist");
			return body;
		}
