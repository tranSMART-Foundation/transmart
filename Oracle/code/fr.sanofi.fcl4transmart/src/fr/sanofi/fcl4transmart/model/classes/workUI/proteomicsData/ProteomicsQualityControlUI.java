...
import fr.sanofi.fcl4transmart.controllers.listeners.proteomicsData.QCListener;
...
/**
 *This class allows the creation of the composite for proteomics data quality control
 */
public class ProteomicsQualityControlUI implements WorkItf{
	...
	public ProteomicsQualityControlUI(DataTypeItf dataType){
	...
	public Composite createUI(Composite parent){
		...
		new Thread(){
			public void run() {
				number=String.valueOf(RetrieveData.getProteomicsProbeNumber(dataType.getStudy().toString()));
		...
		transcriptNumber.setText("Peptide number: "+this.number);
		...
		transcriptLabel.setText("Peptide identifier: ");
	...
	public Composite createBody(String transcript){
		...
		for(File rawFile: ((ProteomicsData)this.dataType).getRawFiles()){
			transcripts.addAll(FileHandler.getPeptide(rawFile, ((ProteomicsData)this.dataType).getColumnMappingFile()));
		}
		if(!transcripts.contains(transcriptId)){
			Label label=new Label(body, SWT.NONE);
			label.setText("This peptide identifier does not exist");
			return body;
		}
