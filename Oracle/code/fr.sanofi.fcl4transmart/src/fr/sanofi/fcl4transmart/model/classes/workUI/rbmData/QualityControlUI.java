...
import fr.sanofi.fcl4transmart.controllers.listeners.rbmData.QCListener;
...
/**
 *This class allows the creation of the composite for rbm data quality control
 */
public class QualityControlUI implements WorkItf{
	...
	public Composite createUI(Composite parent){
		...
		new Thread(){
			public void run() {
				number=String.valueOf(RetrieveData.getRbmProbeNumber(dataType.getStudy().toString()));
		...
		transcriptNumber.setText("Antigen number: "+this.number);
		...
		transcriptLabel.setText("Antigen identifier: ");
	...
	public Composite createBody(String transcript){
		...
		for(File rawFile: ((RbmData)this.dataType).getRawFiles()){
			transcripts.addAll(FileHandler.getAntigens(rawFile));
		}
		if(!transcripts.contains(transcriptId)){
			Label label=new Label(body, SWT.NONE);
			label.setText("This antigen identifier does not exist");
			return body;
		}
