...
public class CheckDataUI implements WorkItf {
	...
	public CheckDataUI(DataTypeItf dataType){
		...
		new Thread(){
			public void run() {
				try{
					...
					tables.add("DEAPP.DE_SNP_DATASET_LOC");
					tables.add("DEAPP.DE_SUBJECT_SNP_DATASET");
					tables.add("DEAPP.DE_SNP_PROBE_SORTED_DEF");
					tables.add("DEAPP.DE_SNP_SUBJECT_SORTED_DEF");
					tables.add("DEAPP.DE_SNP_CALLS_BY_GSM");
					tables.add("DEAPP.DE_SNP_DATA_BY_PATIENT");
					tables.add("DEAPP.DE_SNP_DATA_BY_PROBE");
					...
					int sampleNumber=FileHandler.getSamplesIdForSnp(((SnpData)dataType).getRawFile()).size();
					int probeNumber=FileHandler.getProbeNumberForSnp(((SnpData)dataType).getRawFile());
					
					//get the number of lines for DE_SNP_DATASET_LOC
					int nI=sampleNumber;
					int nE=RetrieveData.getDatasetLoc(dataType.getStudy().toString());
					...
					//get the number of lines for DE_SUBJECT_SNP_DATASET
					nI=sampleNumber;
					nE=RetrieveData.getSubjectSnp(dataType.getStudy().toString());
					...
					//get the number of lines for DE_SNP_PROBE_SORTED_DEF
					nI=25; //number of chromosomes
					nE=RetrieveData.getProbeSortSnp(FileHandler.getSnpGpl(((SnpData)dataType).getAnnotationProps()));
					...
					//get the number of lines for DE_SNP_SUBJECT_SORTED_DEF
					nI=sampleNumber; 
					nE=RetrieveData.getSubjectSortSnp(dataType.getStudy().toString());
					...
					//get the number of lines for DE_SNP_CALLS_BY_GSM
					nI=sampleNumber*probeNumber; 
					nE=RetrieveData.getCallsSnp(dataType.getStudy().toString());
					...
					//get the number of lines for DE_SNP_DATA_BY_PATIENT
					nI=sampleNumber*(FileHandler.getChrNumberFromAnnotation(((SnpData)dataType).getAnnotationFile(), ((SnpData)dataType).getAnnotationProps())+1); 
					nE=RetrieveData.getDataByPatientSnp(dataType.getStudy().toString());
					...
					//get the number of lines for DE_SNP_DATA_BY_PROBE
					nI=FileHandler.getProbesNumberFromAnnotation(((SnpData)dataType).getAnnotationFile(), ((SnpData)dataType).getAnnotationProps()); 
					nE=RetrieveData.getDataByProbeSnp(dataType.getStudy().toString());
