...
import fr.sanofi.fcl4transmart.controllers.listeners.rbmData.CheckAnnotationListener;
import fr.sanofi.fcl4transmart.controllers.listeners.rbmData.RbmLoadAnnotationListener;
...
public class LoadAnnotationUI implements WorkItf, LoadAnnotationUIItf{
	...
	public void addLoadPart(boolean editable){
		...
		if(RetrieveData.testTm_czConnection() && RetrieveData.testTm_lzConnection() && RetrieveData.testDeappConnection()){
			loadButton.addListener(SWT.Selection, new RbmLoadAnnotationListener(this, this.dataType));
		}
		else{
	...
	public void setAnnotationLogFile(File log) {
		((RbmData)this.dataType).setAnnotationFile(log);
	}
}
