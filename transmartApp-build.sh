...
#    ./gradlew clean build publishToMavenLocal  >  $OUT/$PREFIX-$DIR.out 2>&1
    mvn clean install  >   $OUT/$PREFIX-$DIR.out 2>&1
    cd ..
...
if [ $buildwar == 1 ] ; then
    ...
    egrep -v '(warning: Implicitly compiled|warning(-[a-f0-9]*)?[.]gif|error(-[a-f0-9]*)?[.]gif|errors(-[a-f0-9]*)?[.]css|workflowWarnings(-[a-f0-9]*)?[.][a-z]*)$' $OUT/$PREFIX-$DIR.out | egrep -i 'error[^-]|warning|fail'
