...
//		ON a.cat=b.cat ORDER BY a.cat'''
    ...
    def getChildrenWithAccessForUser(String concept_key, AuthUser user) {
        ...
        if (level == -1 && !admin) {
	    //only check on first level of nodes and im not an admin
    ...
    def getConceptPathAccessForUser(List<String> paths, AuthUser user) {
        ...
        if (!admin) {
	    //level of nodes and im not an admin
            logger.trace('NOT AN ADMINISTRATOR CHECKING PERMISSIONS')
    ...
    def getAccess(pathswithtokens, user) {
	    ...
	    //if not admin merge the data from the two maps
            ...
            for (key in children.keySet()) {
                ...
                } else if (tokens.containsKey(childtoken))  {
		    //null tokens are assumed to be unlocked
    ...
    def getSecureTokensCommaSeparated(user) {
        ...
        for (v in tokenmap.keySet())  {
	    //have some kind of access to each of these tokens
            logger.trace(v)
            sb.append("'")
            sb.append(v)
            sb.append("',")
