...
)
{
    ...
    plotHeatmap(matrixData, rowLabels, patientcolors, color.range.clamps, as.numeric(pxPerCell), output.file)
}

plotHeatmap <- function(data, rowLabels, colcolors, color.range.clamps, pxPerCell, output.file = "Heatmap") {
    require(Cairo)
    require(gplots)
